...
def create_buy_sell_signal(ticker, test):
    ...
    test.to_csv(tkr + '_buy_sell_flags.csv')
...
def generate_buy_quantity(test,tkr):
    ...
    for i,row in test.iterrows():
        ...
        if test.loc[i,'BUY_SELL'] == 'Buy':
            ...
            t += 1
            continue
    test.to_csv(tkr + '_buy_sell_quantity.csv')
    return test
...
def calculate_total_price(ticker, test):
    ...
    test.to_csv(tkr + '_current_price_data.csv')
...
def IB_Trader(ticker, test):
    ...
    final.to_csv(sym+'_final_trades.csv')
