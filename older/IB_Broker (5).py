...
# save to CSV file
    ...
    df = util.df(allBars)
    df['my_datetime'] = df['date'] - pd.Timedelta(hours=5, minutes=0, seconds=0)
    ib.disconnect()
...
def create_buy_sell_signal(ticker, test):
    ...
    test['my_datetime'] = test['date'] - pd.Timedelta(hours=5, minutes=0, seconds=0)
...
def generate_buy_quantity(test,tkr):
    ...
    for i,row in test.iterrows():
        ...
        if test.loc[i,'BUY_SELL'] == 'Buy':
            ...
            t += 1
            continue
    test.to_csv('C:/Users/XaoGo/Desktop/IBK Codes/log_files/' + tkr + 'xyz.csv'+tkr + '_buy_sell_quantity.csv')
    return test
...
def IB_Trader(ticker, test):
    ...
    for i, row in test.iterrows():
        ...
        if test.loc[i,'BUY_SELL'] == 'Buy':
            ...
            final = pd.concat([final,test])
            #final.to_csv(sym+'_last_sell.csv')
            ...
            final = pd.concat([final,test])
            #final.to_csv(sym+'_no_action_trade.csv')
            #print(final)
