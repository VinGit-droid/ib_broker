...
def create_buy_sell_signal(ticker, test):
    ...
    test = test[['date','open','symbol','close','prev_close','BUY_SELL']]
    return test

#-------------------PHASE I AND II
def generate_buy_quantity(test):
    ...
    test = create_buy_sell_signal(ticker = 'AAPL', test= test)
    test['Buy_Qty'] = 0
    n = 1
    t = 1
...
def calculate_total_price(ticker, test):
    ...
    test = generate_buy_quantity(test)
