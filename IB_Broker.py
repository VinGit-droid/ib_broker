    ...
    for i, row in test.iterrows():
        ...
        if test.loc[i,'BUY_SELL'] == 'Buy':
            ...
            if Total_Price < bal:
                ...
                m = n * 5
                next_trade_amt = m * close
                
                if next_trade_amt > remaining_bal:
                    #Put Stop Loss
                    print('Total_Price for next trade is ', next_trade_amt)
                    print('Available funds for next trade is ', remaining_bal)
                    print('Not enough to buy. So, putting stop loss.')
                    close_price = close
                    tp = close + 0.01 * close
                    sl = close - 0.01 * close
                    ...
                    ib.sleep(2)
                    my_stoploss = StopOrder('SELL', n, sl)
                    trade = ib.placeOrder(contract, my_stoploss)

                    '''
                    ib.qualifyContracts(contract)
                    my_order = ib.bracketOrder('BUY', n, limitPrice=close_price,  takeProfitPrice=tp,  stopLossPrice=sl )
                    for ord in my_order:
                        ib.placeOrder(contract, ord)
                        ib.sleep(2)
                        final = pd.concat([final,test])
                        '''
                elif next_trade_amt <= remaining_bal:
                    print('Total_Price for next trade is ', next_trade_amt)
                    print('Available funds for next trade is ', remaining_bal)
                    print('Enough funds for next trade also. So, putting normal buy.')
                    my_order = MarketOrder('BUY', n)
                    trade = ib.placeOrder(contract, my_order)
                    final = pd.concat([final,test])
...
def check_balance():
    ...
    for av in raw_balance:
        if av.tag == 'AvailableFunds':
            print('Current Balance is',float(av.value))
